...
class Logger(object):
    def __init__(self, data, logscalars=True, save_embed=True):
        ...
        if data["LAI"]:
            ...
            experience = (
                "./logs/" + data["dataset"] + "/" + data["model_arch"] + "/" + str(exp)
            )
    ...
    def save_model(self, model):
        if len(self.logscalars["Accuraccy/mAP"]) > 1:
            if self.logscalars["Accuraccy/mAP"][-1] >= np.max(
                self.logscalars["Accuraccy/mAP"][:-1]
            ):
                torch.save(model.state_dict(), self.savepath + "/" + "best_mAP" + ".pt")
            if self.logscalars["Accuraccy/CMC1"][-1] >= np.max(
                self.logscalars["Accuraccy/CMC1"][:-1]
            ):
    ...
    def write_embeddings(self, features, labels, images, epoch, tag="default"):
        ...
        self.writer.add_embedding(
            features, metadata=labels, label_img=images, global_step=epoch, tag=tag
        )
...
if __name__ == "__main__":
    ...
    dicttest = {
        "Loss/train_total": np.mean([4, 2, 1]),
        "Loss/train_crossentropy": np.mean([12, 32, 3]),
        "Loss/train_triplet": np.mean([223, 11]),
        "Loss/ce_loss_weight": np.mean([[2, 8, 6, 4, 4]]),
