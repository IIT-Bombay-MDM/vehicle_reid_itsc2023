...
from data.triplet_sampler import (
    CustomDataSet4VehicleID,
    CustomDataSet4Veri776,
    CustomDataSet4Veri776_withviewpont,
    CustomDataSet4VERIWILD,
...
from processor import get_model, test_epoch, train_epoch
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hflip", default=None, type=float, help="Probabilty for horizontal flip"
    )
    parser.add_argument(
        "--randomerase", default=None, type=float, help="Probabilty for random erasing"
    )
    parser.add_argument(
        "--dataset",
        default=None,
        help="Choose one of [Veri776, VERIWILD, Market1501, VehicleID]",
    )
    ...
    parser.add_argument(
        "--softmax_loss", default=None, help="The loss used for classification"
    )
    parser.add_argument(
        "--metric_loss", default=None, help="The loss used as metric loss"
    )
    parser.add_argument(
        "--triplet_margin",
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help="multiplier of the classification loss",
    )
    parser.add_argument(
        "--lambda_triplet",
        default=None,
        type=float,
        help="multiplier of the metric loss",
    )

    parser.add_argument(
        "--parallel",
        default=None,
        help="Whether to used DataParallel for multi-gpu in one device",
    )
    ...
    if args.triplet_margin is not None:
        data["triplet_margin"] = args.triplet_margin
    ...
    if data["dataset"] == "VehicleID":
        ...
        data_train = DataLoader(
            ...
            sampler=RandomIdentitySampler(
                data_train, data["BATCH_SIZE"], data["NUM_INSTANCES"]
            ),
    ...
    if data["dataset"] == "Veri776":
        ...
        if data["LAI"]:
            data_train = CustomDataSet4Veri776_withviewpont(
                data["train_list_file"],
                data["train_dir"],
                data["train_keypoint"],
                data["test_keypoint"],
                is_train=True,
            ...
            data_train = CustomDataSet4Veri776(
                data["train_list_file"],
                data["train_dir"],
                is_train=True,
                transform=train_transform,
    ...
    metric_loss = triplet_loss_fastreid(
        data["triplet_margin"],
        norm_feat=data["triplet_norm"],
