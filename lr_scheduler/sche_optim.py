...
class WarmupCosineLR(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_epochs:
            lr = [
                base_lr * (self.last_epoch + 1) / (self.warmup_epochs + 1e-32)
                for base_lr in self.base_lrs
            ]
        else:
            lr = [
...
class CosineStepLR(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch < self.max_epochs - self.step_epochs:
            lr = [
                ...
                * (
                    ...
                    + math.cos(
                        math.pi
                        * (self.last_epoch)
                        / (self.max_epochs - self.step_epochs)
                    )
                )
                / 2
                for base_lr in self.base_lrs
            ...
            lr = [
                self.gamma ** (self.step_epochs - self.max_epochs + self.last_epoch + 1)
                * base_lr
...
def make_warmup_scheduler(
    ...
    if scheduler_name == "Warm_MultiStep":
        ...
        scheduler = WarmupCosineLR(
            optimizer, max_epochs, warmup_iters, last_epoch=last_epoch, eta_min=min_lr
        )
    elif scheduler_name == "CosineAnnealingLR":
        scheduler = CosineAnnealingLR(
            optimizer, max_epochs, warmup_iters, eta_min=min_lr, delay=delay
        )
