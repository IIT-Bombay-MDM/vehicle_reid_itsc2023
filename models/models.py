...
class MHSA(nn.Module):
    ...
    def forward(self, x):
        ...
        content_position = (
            ...
            .view(1, self.heads, torch.div(C, self.heads, rounding_mode="floor"), -1)
            .permute(0, 1, 3, 2)
        )
...
class Bottleneck_Transformer(nn.Module):
    ...
    def __init__(
        self, in_planes, planes, stride=1, heads=4, resolution=None, use_mlp=False
    ):
        ...
        self.conv2.append(
            MHSA(
                planes, width=int(resolution[0]), height=int(resolution[1]), heads=heads
            )
        )
        if stride == 2:
...
class ClassBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.return_f:
            ...
            if self.circle:
                x = F.normalize(x)
                self.classifier[0].weight.data = F.normalize(
                    self.classifier[0].weight, dim=1
                )
                x = self.classifier(x)
                return x, f
            else:
...
class GroupNormwMomentum(nn.Module):
    def __init__(self, n_groups, n_channels):
        ...
        for i in range(n_groups):
            self.gn.append(
                nn.BatchNorm2d(int(n_channels / 2)).apply(weights_init_kaiming)
            )
...
class Conv_MHSA_2G(nn.Module):
    def __init__(self, c_in, c_out, resolution=[16, 16], heads=4) -> None:
        ...
        self.conv2 = nn.Conv2d(
            c_in // 2, c_out // 2, kernel_size=3, stride=1, padding=1, bias=False
        )
        self.MHSA_1 = MHSA(
            c_out // 2, width=int(resolution[0]), height=int(resolution[1]), heads=heads
        )
...
class Conv_MHSA_4G(nn.Module):
    def __init__(self, c_in, c_out, resolution=[16, 16], heads=4) -> None:
        ...
        self.MHSA_1 = MHSA(
            c_out // 4, width=int(resolution[0]), height=int(resolution[1]), heads=heads
        )
        self.MHSA_2 = MHSA(
            c_out // 4, width=int(resolution[0]), height=int(resolution[1]), heads=heads
        )
    ...
    def __init__(self, backbone="ibn", stride=1):
        ...
        if backbone == "r50":
            ...
            model_ft = torch.hub.load(
                "XingangPan/IBN-Net", "resnet101_ibn_a", pretrained=True
            )  # 'resnet50_ibn_a'
        elif backbone == "34ibn":
            model_ft = torch.hub.load(
                "XingangPan/IBN-Net", "resnet34_ibn_a", pretrained=True
            )  # 'resnet50_ibn_a'
        else:
            model_ft = torch.hub.load(
...
class multi_branches(nn.Module):
    def __init__(
        ...
        if n_groups > 0:
            convlist = [
                k.split(".")
                for k, m in model_ft.named_modules(remove_duplicate=False)
            ...
            for item in convlist:
                ...
                weight = m.weight[
                    : int(m.weight.size(0)), : int(m.weight.size(1) / n_groups), :, :
                ]
                if end_bot_g and item[1] == "conv2":
                    setattr(
                        model_ft[int(item[0])], item[1], MHSA_2G(int(512), int(512))
                    )
                elif group_conv_mhsa and item[1] == "conv2":
                    setattr(
                        model_ft[int(item[0])],
                        item[1],
                        Conv_MHSA_4G(int(512), int(512)),
                    )
                elif group_conv_mhsa_2 and item[1] == "conv2":
                    setattr(
                        model_ft[int(item[0])],
                        item[1],
                        Conv_MHSA_2G(int(512), int(512)),
                    )
                else:
                    if item[1] == "downsample":
                        getattr(model_ft[int(item[0])], item[1])[0] = nn.Conv2d(
                            int(m.weight.size(1)),
                            int(m.weight.size(0)),
                            kernel_size=1,
                            stride=1,
                        ...
                        if pretrain_ongroups:
                            getattr(model_ft[int(item[0])], item[1])[
                                0
                            ].weight.data = weight
                    elif item[1] == "conv2":
                        setattr(
                        ...
                        if pretrain_ongroups:
                            setattr(
                                model_ft[int(item[0])].get_submodule(item[1]).weight,
                                "data",
                                weight,
                            )
                    else:
            ...
            if n_branches[0] == "2x":
                ...
                for item in n_branches:
                    if item == "R50":
                        ...
                        layer_0 = Bottleneck_Transformer(
                            1024, 512, resolution=[16, 16], use_mlp=False
                        )
                        layer_1 = Bottleneck_Transformer(
                            2048, 512, resolution=[16, 16], use_mlp=False
                        )
                        layer_2 = Bottleneck_Transformer(
                            2048, 512, resolution=[16, 16], use_mlp=False
                        )
                        self.model.append(nn.Sequential(layer_0, layer_1, layer_2))
                    else:
                        print(
                            "No valid architecture selected for branching by expansion!"
                        )
        else:
            self.model.append(model_ft)
        ...
        if self.withLAI:
            ...
            if n_cams > 0 and n_views > 0:
                if x2g or x4g:
                    ...
                    self.LAI = nn.Parameter(
                        torch.zeros(len(n_branches), n_cams * n_views, 2048)
                    )
            elif n_cams > 0:
                if x2g or x4g:
    ...
    def forward(self, x, cam, view):
        ...
        for i in range(len(x)):
            ...
            for j in range(self.n_groups):
                aux_emb = emb[
                    :,
                    int(2048 / self.n_groups * j) : int(2048 / self.n_groups * (j + 1)),
                ]
                if self.LBS:
                    if (i + j) % 2 == 0:
                    ...
                    aux_emb = emb[
                        :,
                        int(2048 / self.n_groups * j) : int(
                            2048 / self.n_groups * (j + 1)
                        ),
                    ]
                    pred, ff = self.finalblocks[i + j](aux_emb)
                    embs.append(aux_emb)
...
if __name__ == "__main__":
    ...
    ### MBR_4B
    model = MBR_model(
        575, ["R50", "R50", "BoT", "BoT"], n_groups=0, losses="LBS", LAI=True
    )
    preds, embs, ffs, output = model(
        input, torch.randint(0, 19, (32, 1)), torch.randint(0, 7, (32, 8))
    )
    print("\nn_preds: ", len(preds))
