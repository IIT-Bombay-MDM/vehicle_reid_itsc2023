...
def test_epoch(
    ...
    # blend_ratio = 0.3
    with torch.no_grad():
        for image, q_id, cam_id, view_id in tqdm(
            dataloader_q, desc="Query infer (%)", bar_format="{l_bar}{bar:20}{r_bar}"
        ):
    ...
    cmc, mAP = eval_func(
        distmat, q_vids, g_vids, q_camids, g_camids, remove_junk=remove_junk
    )
...
if __name__ == "__main__":
    ...
    if data["dataset"] == "VERIWILD2.0":
        ...
        data_g = CustomDataSet4VERIWILDv2(
            vw2_dir + "test_split_V2/" + set + "_gallery.txt",
            vw2_dir,
            transform=teste_transform,
            with_view=True,
        )
        data_q = DataLoader(
            data_q, batch_size=data["BATCH_SIZE"], shuffle=False, num_workers=0
        )  # data['BATCH_SIZE'] data['num_workers_teste']
        data_g = DataLoader(
            data_g, batch_size=data["BATCH_SIZE"], shuffle=False, num_workers=0
        )
    ...
    if data["dataset"] == "VehicleID":
        ...
        for i in range(10):
            ...
            data_q = CustomDataSet4VehicleID_Random(
                lines,
                data["ROOT_DIR"],
                is_train=False,
                mode="q",
            ...
            data_g = CustomDataSet4VehicleID_Random(
                lines,
                data["ROOT_DIR"],
                is_train=False,
                mode="g",
