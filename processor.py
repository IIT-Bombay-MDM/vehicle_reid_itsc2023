...
def train_epoch(
    ...
    acc_v = 0
    stepcount = 0
    for image_batch, label, cam, view in tqdm(
        dataloader,
    ...
    with torch.no_grad():
        ...
        if epoch == 119:
            writer.write_embeddings(
                torch.cat(qf).cpu(),
                torch.cat(q_vids).cpu(),
                torch.cat(q_images) / 2 + 0.5,
                120,
